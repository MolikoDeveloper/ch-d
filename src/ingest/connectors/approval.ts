...
type Entry={pollster:Pollster;date:string;url:string;approval:number|null;disapproval:number|null;subject:string|null;methodology:string;snapshotId:number};
...
function measures(text:string){
  ...
  const subjectMatch=clean(text).match(/Presidente\s+([A-ZÁÉÍÓÚÑ][A-Za-zÁÉÍÓÚÑáéíóúñ.-]+(?:\s+[A-ZÁÉÍÓÚÑ][A-Za-zÁÉÍÓÚÑáéíóúñ.-]+){0,3})/);return{approval,disapproval,subject:subjectMatch?.[1]??null};
}
...
async function discover(pollster:Pollster,run:number){
  ...
  return urls.slice(0,Math.max(1,Number(process.env.APPROVAL_PAGE_LIMIT??60)))
}
async function parseEntry(pollster:Pollster,url:string,run:number):Promise<Entry|null>{const snap=await fetchAndSnapshot(pollster,run,url),text=clean(snap.text),date=dateFrom(text)||dateFrom(url);if(!date)return null;const m=measures(text);if(m.approval==null&&m.disapproval==null)return null;return{pollster,date,url,approval:m.approval,disapproval:m.disapproval,subject:m.subject,methodology:SOURCES[pollster].methodology,snapshotId:snap.snapshotId}}

async function syncPollster(pollster:Pollster,country:number){
  const run=startRun(pollster);let seen=0,written=0,failed=0;
  try{
    const urls=await discover(pollster,run),approvalMetric=upsertMetric(pollster,"approval"),disapprovalMetric=upsertMetric(pollster,"disapproval");console.log(`[approval] ${pollster}: ${urls.length} publicaciones candidatas`);
    const stmt=db.prepare(`INSERT INTO observations(source_id,external_id,observed_at,metric,value_number,value_text,unit,geo_area_id,subject_type,subject_id,raw_snapshot_id,payload_json) VALUES(?,?,?,?,?,NULL,'%',?,'office','president',?,?) ON CONFLICT(source_id,external_id,metric) DO UPDATE SET value_number=excluded.value_number,raw_snapshot_id=excluded.raw_snapshot_id,payload_json=excluded.payload_json`);
    for(let i=0;i<urls.length;i++){try{const entry=await parseEntry(pollster,urls[i],run);seen++;if(!entry)continue;const payload=JSON.stringify({pollster,date:entry.date,subject:entry.subject,sourceUrl:entry.url,methodology:entry.methodology,official:false});if(entry.approval!=null){stmt.run(pollster,`${pollster}:${entry.date}:approval`,entry.date,approvalMetric,entry.approval,country,entry.snapshotId,payload);written++}if(entry.disapproval!=null){stmt.run(pollster,`${pollster}:${entry.date}:disapproval`,entry.date,disapprovalMetric,entry.disapproval,country,entry.snapshotId,payload);written++}}catch(e){failed++;console.warn(`[approval] ${pollster} ${urls[i]}: ${String(e)}`)}if((i+1)%10===0||i+1===urls.length)updateRunProgress(run,seen,written,`${i+1}/${urls.length}; failed=${failed}`)}
    finishRun(run,written?"success":"failed",`observations=${written}; failed=${failed}`,seen,written);return{pollster,seen,written,failed};
  }catch(e){finishRun(run,"failed",String(e),seen,written);throw e}
}

export async function syncApproval(){
  ensureSources();const country=countryGeoAreaId(),results=[] as Array<{pollster:Pollster;seen:number;written:number;failed:number}>,errors:string[]=[];
  for(const pollster of Object.keys(SOURCES) as Pollster[])try{results.push(await syncPollster(pollster,country))}catch(e){errors.push(`${pollster}: ${String(e)}`)}
  const written=results.reduce((n,r)=>n+r.written,0);if(!written&&errors.length)throw new Error(errors.join("; "));return{results,written,errors};
}
