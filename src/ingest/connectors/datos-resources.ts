  ...
  try{
    await pool(resources,concurrency,async(resource,index)=>{
      ...
      if((index+1)%25===0 || index+1===resources.length){
        ...
        console.log(`[datos.gob] ${index+1}/${resources.length} recursos · parsed=${parsed} unsupported=${unsupported} failed=${failed} · ${((index+1)/elapsed).toFixed(2)} rec/s`);
      }
    });
    const status=failed===resources.length&&resources.length>0?"failed":"success";
    const message=`resources=${resources.length}; parsed=${parsed}; downloaded=${downloaded}; unsupported=${unsupported}; retired=${retired}; failed=${failed}`;
    finishRun(run,status,message,seen,written);
    return {resources:resources.length,seen,written,parsed,downloaded,unsupported,retired,failed,concurrency};
