...
type ResourceStatus="parsed"|"downloaded"|"unsupported";
...
export async function syncDatosGobResourceByRow(resource:DatosResourceRow,run:number,opts:{includeRaw?:boolean}={}){
  ...
  if(badUrl){
    markUnsupported(resource.id,badUrl);
    return {seen,written,status:"unsupported" as ResourceStatus,reason:badUrl};
  }
  ...
  try{
    ...
    if(!parseable && !opts.includeRaw){
      const reason=`Formato no estructurado todavía: ${resource.format||'desconocido'}`;
    ...
    tx();
    db.prepare(`UPDATE source_resources SET sync_status=?,last_success_at=?,last_error=NULL,skip_reason=NULL,last_snapshot_id=?,last_sha256=?,record_count=(SELECT count(*) FROM source_records WHERE resource_id=?) WHERE id=?`)
      .run(parseable?"parsed":"downloaded",new Date().toISOString(),snap.snapshotId,snap.sha256,resource.id,resource.id);
    return {seen,written,status:(parseable?"parsed":"downloaded") as ResourceStatus,sha256:snap.sha256};
  }catch(e){
    const permanent=permanentFailureReason(e);
    if(permanent){
      markUnsupported(resource.id,permanent,e);
      return {seen,written,status:"unsupported" as ResourceStatus,reason:permanent};
    }
