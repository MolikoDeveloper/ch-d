...
function numeric(v:unknown){if(typeof v==="number")return Number.isFinite(v)?v:null;const s=String(v??"").trim().replace(/\./g,"").replace(",",".");if(!s)return null;const n=Number(s);return Number.isFinite(n)?n:null}
...
function upsertMetric(kind:EntityKind,scope:Scope,key:string,measure:"count"|"pct",metadata:Record<string,unknown>){
  const id=metricId(kind,scope,key,measure),unit=measure==="pct"?"%":kind==="persons"?"personas":"hogares";
...
function ensureUnitVecinal(code:string,parentId:number){
  ...
  return Number((db.query(`SELECT id FROM geo_areas WHERE source_id='rsh' AND external_id=?`).get(ext)as any).id);
}
...
function writeObservation(args:{kind:EntityKind;scope:Scope;geoId:number;subjectId:string;period:string;key:string;count:number;total:number;snapshotId:number;resource:PackageResource;derived?:boolean}){
  ...
  const countMetric=upsertMetric(kind,scope,key,"count",common);
  const pctMetric=upsertMetric(kind,scope,key,"pct",common);
  const stmt=db.prepare(`INSERT INTO observations(source_id,external_id,observed_at,metric,value_number,value_text,unit,geo_area_id,subject_type,subject_id,raw_snapshot_id,payload_json)
    VALUES('rsh',?,?,?,?,?,?,?,?,?,?,?)
...
function processScope(kind:EntityKind,scope:Scope,row:Row,geoId:number,subjectId:string,period:string,snapshotId:number,resource:PackageResource){
  ...
  const lower=["0-40","41-50","51-60","61-70"].reduce((a,k)=>a+(counts.get(k)??0),0);
  const upper=["71-80","81-90","91-100"].reduce((a,k)=>a+(counts.get(k)??0),0);
  ...
  const totalMetric=upsertMetric(kind,scope,"total","count",{dataset:kind,scope,resourceId:resource.id??null});
  db.prepare(`INSERT INTO observations(source_id,external_id,observed_at,metric,value_number,value_text,unit,geo_area_id,subject_type,subject_id,raw_snapshot_id,payload_json)
    VALUES('rsh',?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(source_id,external_id,metric) DO UPDATE SET observed_at=excluded.observed_at,value_number=excluded.value_number,geo_area_id=excluded.geo_area_id,raw_snapshot_id=excluded.raw_snapshot_id,payload_json=excluded.payload_json`)
    .run(`${kind}:${scope}:${subjectId}:${period}:total`,period,totalMetric,total,null,kind==="persons"?"personas":"hogares",geoId,`rsh_${scope}`,subjectId,snapshotId,JSON.stringify({dataset:kind,scope,resourceId:resource.id??null}));
  return written+1;
}
  ...
  try{
    ...
    for(let d=0;d<DATASETS.length;d++){
      ...
      try{
        ...
        const snap=await fetchAndSnapshot("rsh",run,String(resource.url)),parsed=parseCsv(snap.text),fallback=fallbackPeriod(resource);
        const seenCommunes=new Set<string>();
        db.transaction(()=>{
          for(const row of parsed){
            seen++;
            const communeCode=normalizeCode(valueBy(row,["comuna"]),5);if(!communeCode)continue;
            const communeGeo=resolveGeoArea(communeCode,null);if(!communeGeo)continue;
            const period=periodDate(valueBy(row,["periodo"]),fallback);
            if(!seenCommunes.has(communeCode)){
              written+=processScope(dataset.kind,"commune",row,communeGeo,communeCode,period,snap.snapshotId,resource);
              seenCommunes.add(communeCode);communes++;
            }
            const uv=uvCode(valueBy(row,["unidad vecinal"]));if(!uv)continue;
            const uvGeo=ensureUnitVecinal(uv,communeGeo);
            written+=processScope(dataset.kind,"unit_vecinal",row,uvGeo,uv,period,snap.snapshotId,resource);units++;
          }
        })();
