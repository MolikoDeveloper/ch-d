...
type CkanResource = { id:string; name?:string; format?:string; url:string; datastore_active?:boolean; [k:string]:unknown };
    ...
    for(let start=0;;start+=rows){
      ...
      for(const ds of body.result.results){
        ...
        for(const r of ds.resources ?? []){
          db.prepare(`INSERT INTO source_resources(catalog_item_id,external_id,name,format,url,datastore_active,metadata_json)
            VALUES(?,?,?,?,?,?,?)
            ON CONFLICT(catalog_item_id,external_id) DO UPDATE SET name=excluded.name,format=excluded.format,url=excluded.url,datastore_active=excluded.datastore_active,metadata_json=excluded.metadata_json`)
            .run(item,r.id,r.name ?? null,(r.format ?? "").toUpperCase(),r.url,r.datastore_active?1:0,JSON.stringify(r));
        }
        written++;
      }
