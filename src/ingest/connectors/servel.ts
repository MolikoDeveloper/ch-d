...
function anchors(html:string,base:string){const out:Link[]=[];for(const m of html.matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)){try{out.push({url:new URL(decode(m[1]),base).toString(),text:decode(m[2])})}catch{}}return out}
...
export async function syncServel(){
  ...
  try{
    ...
    const uniquePages=[...new Map(pageLinks.map(x=>[x.url,x])).values()].slice(0,Number(process.env.SERVEL_PAGE_LIMIT??50));
    const resources:Array<{url:string;text:string;page:string;process:string}>=[];
    for(let i=0;i<uniquePages.length;i++){
      const page=uniquePages[i];try{const snap=await fetchAndSnapshot("servel",run,page.url);pages++;for(const l of anchors(snap.text,page.url)){const f=format(l.url);if(f&&/\.(?:xlsx?|csv|zip|txt)(?:\?|$)/i.test(l.url))resources.push({url:l.url,text:l.text||l.url,page:page.url,process:page.text})}}catch(e){console.error(`[servel] página ${page.url}: ${String(e)}`)}
    }
    const selected=[...new Map(resources.map(r=>[r.url,r])).values()].slice(0,Number(process.env.SERVEL_RESOURCE_LIMIT??100));
    ...
    for(let i=0;i<selected.length;i++){
      const r=selected[i];try{
        const f=format(r.url),snap=await fetchAndSnapshot("servel",run,r.url),rows=parseResource(f,snap.bytes,snap.text),y=yearOf(r.process,r.text,r.url);if(!y)continue;let local=0;
        db.transaction(()=>{for(const row of rows){if((row as any).__parse_error)continue;seen++;const geo=commune(row);if(!geo.geoId)continue;for(const[header,raw]of Object.entries(row)){if(isGeoOrDimension(header)||!isParticipation(header))continue;const n=numeric(raw);if(!n)continue;const u=unit(header,raw),id=metricId(header,r.process);
          ...
          obs.run(`${geo.code||geo.name}:${id}:${y}`,String(y),id,n.value,u,geo.geoId,"commune",geo.code||geo.name,snap.snapshotId,JSON.stringify({process:r.process,resource:r.text,year:y}));written++;local++}}})();resourcesDone++;console.log(`[servel] ${i+1}/${selected.length} ✓ ${r.process} · obs=${local}`)
