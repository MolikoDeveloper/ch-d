...
function numeric(v:unknown){if(typeof v==="number")return Number.isFinite(v)?v:null;let s=clean(v);if(!s||/^[-*]+$/.test(s)||/^(?:n\/?a|s\/?i|sin dato)$/i.test(s))return null;const pct=s.includes("%");s=s.replace(/%/g,"").replace(/\.(?=\d{3}(?:\D|$))/g,"").replace(",",".");const n=Number(s);return Number.isFinite(n)?{value:n,pct}:null}
...
function slug(v:string){return norm(v).replace(/\s+/g,"-")}
...
function headers(row:Row){const seen=new Map<string,number>();return row.map((v,i)=>{let h=clean(v)||`col_${i+1}`;const base=h;const n=(seen.get(base)??0)+1;seen.set(base,n);if(n>1)h=`${h} ${n}`;return h})}
...
function metricTitle(resource:Resource,header:string){const r=theme(resource);const n=norm(header);if(/^(estimacion|tasa|porcentaje|incidencia|indice|valor)$/.test(n))return r.label;return `${r.label} — ${clean(header)}`}
...
async function packageSearch(run:number,q:string){const url=`${CKAN}/package_search?q=${encodeURIComponent(q)}&rows=100`;const snap=await fetchAndSnapshot("casen",run,url);const json=JSON.parse(snap.text) as any;if(!json?.success)throw new Error(`CKAN package_search falló: ${q}`);return(json.result?.results??[]) as Package[]}

function resolveCommune(row:Record<string,unknown>){let communeCode="",communeName="";for(const[k,v]of Object.entries(row)){const n=norm(k);if((n.includes("codigo")||n.includes("cut"))&&n.includes("comuna"))communeCode=code(v)||communeCode;else if(n==="comuna"||n.endsWith(" comuna"))communeName=clean(v)||communeName}return{communeCode,communeName,geoId:communeCode?resolveGeoArea(communeCode,null):communeName?resolveGeoArea(communeName,null):null}}
...
export async function syncCasen(){
  ...
  try{
    ...
    const packages=[...new Map(found.filter(p=>yearOf(p)).map(p=>[p.name??p.title??crypto.randomUUID(),p])).values()].sort((a,b)=>(yearOf(a)??0)-(yearOf(b)??0));
    ...
    for(let ri=0;ri<resources.length;ri++){
      ...
      try{
        ...
        const snap=await fetchAndSnapshot("casen",run,String(resource.url));
        const wb=XLSX.read(snap.bytes,{type:"array",cellDates:false,raw:false});let local=0;
        db.transaction(()=>{
          for(const sheetName of wb.SheetNames){
            const matrix=XLSX.utils.sheet_to_json<Row>(wb.Sheets[sheetName],{header:1,defval:null,raw:false}) as Row[];
            const hi=findHeader(matrix);if(hi<0)continue;const hs=headers(matrix[hi]);
            for(const arr of matrix.slice(hi+1)){
              if(!arr?.some(v=>clean(v)))continue;const row:Object=Object.fromEntries(hs.map((h,i)=>[h,arr[i]??null]));seen++;
              const geo=resolveCommune(row as Record<string,unknown>);if(!geo.geoId)continue;
              for(const [header,raw] of Object.entries(row as Record<string,unknown>)){
                if(isGeoHeader(header))continue;const parsed=numeric(raw);if(!parsed)continue;
                const id=metricId(resource,header),r=role(header),t=theme(resource),unit=parsed.pct||r!=="other"?"%":null;
                metric.run(id,metricTitle(resource,header),`${clean(resource.name)}. Estimación comunal oficial publicada por MDSF/BIDAT.`,"social",t.label,unit,"annual",JSON.stringify({dataset:pkg.name,title:pkg.title,resourceId:resource.id,resourceName:resource.name,year,role:r,mapEligible:r==="estimate"}));
                const external=`${geo.communeCode||geo.communeName}:${id}:${year}`;
                obs.run(external,String(year),id,parsed.value,unit,geo.geoId,"commune",geo.communeCode||geo.communeName,snap.snapshotId,JSON.stringify({sheet:sheetName,resourceId:resource.id,role:r,year}));written++;local++;
              }
            }
          }
        })();
    ...
    finishRun(run,resourcesDone?"success":"failed",`resources=${resourcesDone}; failed=${failed}`,seen,written);
    if(!resourcesDone)throw new Error("No se pudo sincronizar ningún recurso CASEN comunal");
