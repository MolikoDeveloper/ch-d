...
function hoursLabel(hours:number){if(hours>=24&&hours%24===0)return`${hours/24}d`;return`${hours}h`}
function ageLabel(ms:number){const minutes=Math.max(0,Math.floor(ms/60000));if(minutes<60)return`${minutes} min`;const hours=Math.floor(minutes/60);if(hours<48)return`${hours} h`;return`${Math.floor(hours/24)} d`}
...
export async function syncAll(options:SyncAllOptions){
  ...
  async function step(source:string,fn:()=>Promise<unknown>,skip?:string){if(skip){console.log(`\n[sync] ${source}: omitido · ${skip}`);results.push({source,status:"skipped",detail:skip});return}console.log(`\n[sync] ── ${source} ──`);const t=performance.now();try{const result=await fn(),seconds=((performance.now()-t)/1000).toFixed(1);console.log(`[sync] ${source}: listo en ${seconds}s`);if(result!==undefined)console.log(result);results.push({source,status:"ok",detail:`${seconds}s`})}catch(e){const message=String(e);console.error(`[sync] ${source}: ERROR · ${message}`);results.push({source,status:"failed",detail:message.slice(0,160)})}}
  ...
  await step("SERVEL / padrón y resultados",()=>syncServel(),freshnessSkip("servel",options));
  ...
  const today=new Date().toISOString().slice(0,10),from=options.from??process.env.SYNC_CHILECOMPRA_FROM??today,to=options.to??process.env.SYNC_CHILECOMPRA_TO??from,explicitChileCompra=Boolean(options.from||options.to||process.env.SYNC_CHILECOMPRA_FROM||process.env.SYNC_CHILECOMPRA_TO),ccSkip=!process.env.CHILECOMPRA_TICKET?.trim()?"falta CHILECOMPRA_TICKET":explicitChileCompra?undefined:freshnessSkip("chilecompra",options);
  ...
  const seconds=((performance.now()-started)/1000).toFixed(1);console.log(`\n[sync] Finalizado en ${seconds}s`);console.table(results);const failed=results.filter(x=>x.status==="failed").length,skipped=results.filter(x=>x.status==="skipped").length;console.log(`[sync] ok=${results.length-failed-skipped} failed=${failed} skipped=${skipped}`);if(failed)process.exitCode=1;return results;
}
