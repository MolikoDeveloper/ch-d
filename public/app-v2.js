...
function initMap(){
  ...
  regionLayer=L.layerGroup().addTo(map);communeLayer=L.layerGroup().addTo(map);
  resetMap();
}
...
function renderMap(){
  ...
  const metric=metrics.find(x=>x.metric===$('#indicatorSelect').value);
  ...
  for(const feature of regionFeatures){
    const p=feature.properties||{},code=String(p.code||'');
    ...
    layer.on('click',()=>selectRegion(code,true));
    layer.addTo(regionLayer);
  }

  const visible=filteredCommunes(false);
  for(const c of visible){
    if(c.centroid_lat==null||c.centroid_lon==null)continue;
    let valueEntry=communeValues.get(String(c.code));
    if(mode==='indicator'&&indicatorData&&values.some(v=>v.geo_type==='commune')&&!valueEntry)continue;
    const v=valueEntry?.value_number;
    const t=mode==='indicator'?intensity(v,min,max):null;
    const selected=selectedCommune===String(c.code);
...
function filteredCommunes(useSearch=true){
  const q=useSearch?normalize($('#territorySearch').value):'';
  return territories.communes.filter(c=>(!selectedRegion||String(c.region_code)===String(selectedRegion))&&(!q||normalize(`${c.name} ${c.region_name} ${c.code}`).includes(q)));
}
function populateRegions(){
  const select=$('#regionSelect');select.replaceChildren(el('option','Todas las regiones'));select.firstChild.value='';
  for(const r of territories.regions){const o=el('option',r.name);o.value=r.code;select.appendChild(o)}
}
function populateCommuneSelect(){
  const list=filteredCommunes(false),select=$('#communeSelect');select.replaceChildren(el('option','Selecciona una comuna'));select.firstChild.value='';
  for(const c of list){const o=el('option',`${c.name} · ${c.region_name}`);o.value=c.code;select.appendChild(o)}
  if(selectedCommune&&list.some(c=>String(c.code)===selectedCommune))select.value=selectedCommune;
}
function renderCommuneList(){
  ...
  for(const c of list){const b=el('button',null,`commune-row${selectedCommune===String(c.code)?' selected':''}`);b.type='button';const left=el('span');left.append(el('b',c.name),el('small',`${c.region_name} · ${c.code}`));const right=el('small',`${integer.format(c.indicator_rows||0)} datos`);b.append(left,right);b.addEventListener('click',()=>selectCommune(String(c.code),true));box.appendChild(b)}
...
function selectRegion(code,zoom=false){
  selectedRegion=String(code||'');$('#regionSelect').value=selectedRegion;selectedCommune='';$('#communeSelect').value='';populateCommuneSelect();renderCommuneList();renderMap();
  if(zoom&&selectedRegion){const f=regionFeature(selectedRegion);if(f){const temp=L.geoJSON(f);map.fitBounds(temp.getBounds(),{padding:[20,20]})}}
}
async function selectCommune(code,zoom=false){
  const c=communeByCode(code);if(!c)return;
  selectedCommune=String(code);selectedRegion=String(c.region_code||'');$('#regionSelect').value=selectedRegion;populateCommuneSelect();$('#communeSelect').value=selectedCommune;renderCommuneList();renderMap();
...
function renderCommuneProfile(data){
  ...
  const mayor=data.authorities.find(a=>a.role==='mayor'),councillors=data.authorities.filter(a=>a.role==='councillor');
  const authority=el('section',null,'authority-section');authority.appendChild(el('h4','Autoridades municipales'));
  ...
  box.appendChild(authority);

  const groups=new Map();
  for(const item of data.indicators){const key=item.subcategory||'Otros datos';if(!groups.has(key))groups.set(key,[]);groups.get(key).push(item)}
  for(const [section,items] of groups){
    const details=document.createElement('details');details.className='indicator-section';details.open=true;
    const summary=document.createElement('summary');summary.append(el('b',section),el('span',`${items.length} indicadores`));details.appendChild(summary);
    const table=el('div',null,'indicator-table');
    for(const x of items){const row=el('div',null,'indicator-row'),name=el('div');name.append(el('b',x.title),el('small',`${dateOnly(x.observed_at)}${x.unit?` · ${x.unit}`:''}`));const value=x.value_number!=null?formatValue(x.value_number,x.unit):(x.value_text??'—');row.append(name,el('strong',value));table.appendChild(row)}
    details.appendChild(table);box.appendChild(details);
  }
}

async function loadMetrics(source){
  metrics=await fetch(`/api/explorer/metrics?source=${encodeURIComponent(source)}`).then(r=>r.json());const select=$('#indicatorSelect');select.replaceChildren(el('option','Selecciona un indicador'));select.firstChild.value='';
  const groups=new Map();for(const m of metrics){const key=m.subcategory||'Otros';if(!groups.has(key))groups.set(key,[]);groups.get(key).push(m)}
  for(const [label,items] of groups){const group=document.createElement('optgroup');group.label=label;for(const m of items){const o=el('option',m.title);o.value=m.metric;group.appendChild(o)}select.appendChild(group)}
  const preferred=metrics.find(m=>/poblaci[oó]n comunal/i.test(m.title))||metrics.find(m=>/poblaci/i.test(m.title))||metrics[0];
  if(preferred){select.value=preferred.metric;await loadIndicator(preferred.metric)}else{indicatorData=null;renderMap()}
}
async function loadIndicator(metric){
  if(!metric){indicatorData=null;renderMap();return}
  const source=$('#indicatorSource').value;indicatorData=await fetch(`/api/explorer/map?source=${encodeURIComponent(source)}&metric=${encodeURIComponent(metric)}`).then(r=>r.json());$('#mapMode').value='indicator';renderMap();
}
...
async function loadInitial(){
  const [panorama,territoryData,mapData]=await Promise.all([fetch('/api/panorama').then(r=>r.json()),fetch('/api/explorer/territories').then(r=>r.json()),fetch('/api/map/regions').then(r=>r.json())]);
  territories=territoryData;regionFeatures=mapData.features||[];renderPanorama(panorama);populateRegions();populateCommuneSelect();renderCommuneList();await loadMetrics($('#indicatorSource').value);renderMap();
}
async function refreshOverview(){if(document.hidden)return;const panorama=await fetch('/api/panorama').then(r=>r.json());renderPanorama(panorama)}

initMap();
$('#resetMap').addEventListener('click',resetMap);
$('#regionSelect').addEventListener('change',e=>selectRegion(e.target.value,true));
$('#communeSelect').addEventListener('change',e=>{if(e.target.value)selectCommune(e.target.value,true)});
