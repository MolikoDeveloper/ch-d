...
function renderMap(kind='combined'){
  ...
  const maxObs=Math.max(1,...areas.map(f=>Number(f.properties?.observations)||0));
  for(const f of allFeatures){
    const p=f.properties||{},type=f.geometry?.type;
    if(type==='Polygon'||type==='MultiPolygon'){
      const amount=Number(p.transaction_amount)||0,obs=Number(p.observations)||0;
      const intensity=kind==='transaction'?amount/maxAmount:kind==='observation'?obs/maxObs:Math.max(amount/maxAmount,obs/maxObs);
      const layer=L.geoJSON(f,{style:{weight:1,fillOpacity:kind==='areas'?.12:.12+Math.min(.58,intensity*.58)}});
      layer.bindPopup(document.createTextNode(`${p.label||p.geo_name||'Región'} — ${fmt.format(p.transactions||0)} compras — ${money(amount)} — ${fmt.format(obs)} observaciones`));
    ...
    const marker=L.circleMarker([lat,lon],{radius:5,weight:1,color:'#fff',fillColor:p.kind==='transaction'?'#1769e0':'#d93045',fillOpacity:.82});
...
async function refresh(){const[dash,sources,geo]=await Promise.all([fetch('/api/dashboard').then(r=>r.json()),fetch('/api/sources').then(r=>r.json()),fetch('/api/map/features').then(r=>r.json())]);const t=dash.totals;$('#sources').textContent=fmt.format(t.sources);$('#populatedSources').textContent=`${fmt.format(t.populatedSources||0)} con datos`;$('#datasets').textContent=fmt.format(t.datasets);$('#sourceRecords').textContent=fmt.format(t.sourceRecords);$('#metricCount').textContent=fmt.format(t.metrics);$('#observations').textContent=fmt.format(t.observations);$('#updatedAt').textContent=`Actualizado ${formatDate(dash.generatedAt)}`;$('#syncLabel').textContent=dash.activeRuns?.length?`Sincronizando ${dash.activeRuns.map(r=>r.source_id).join(', ')}`:'Datos locales actualizados';renderSources(sources);renderEconomic(dash.economicSeries||[]);renderChileCompra(dash.chileCompra||{});renderCoverage(dash.sourceCoverage||[]);renderConnectorProgress(dash.connectorProgress||[]);renderRuns(dash.recentRuns||[]);allFeatures=geo.features||[];renderMap($('#mapLayer').value)}
initMap();$('#mapLayer').addEventListener('change',e=>renderMap(e.target.value));refresh().catch(console.error);setInterval(()=>refresh().catch(console.error),15000);
