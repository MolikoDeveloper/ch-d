...
let dashboardTimer=null,progressTimer=null,mapTimer=null;
...
function renderMap(kind='combined'){
  ...
  const maxEnergy=Math.max(1,...areas.map(f=>Number(f.properties?.energy_observations)||0));
  for(const f of allFeatures){
    const p=f.properties||{},type=f.geometry?.type;
    if(type==='Polygon'||type==='MultiPolygon'){
      const amount=Number(p.transaction_amount)||0,energyObs=Number(p.energy_observations)||0;
    ...
    if(kind==='geography'||kind==='energy')continue;
    if(kind!=='combined'&&p.kind!==kind)continue;
    if(type!=='Point')continue;
    const [lon,lat]=f.geometry.coordinates;
    L.circleMarker([lat,lon],{radius:5,weight:1,color:'#fff',fillColor:'#1769e0',fillOpacity:.82}).bindPopup(document.createTextNode([p.label,p.geo_name,p.date?.slice?.(0,10),formatValue(p.value)].filter(Boolean).join(' — '))).addTo(featureLayer);
  }
}
...
function renderEnergy(data){$('#energyMetrics').textContent=fmt.format(data.metrics||0);$('#energyObservations').textContent=fmt.format(data.observations||0);$('#energyRegional').textContent=fmt.format(data.regional||0);const box=$('#energyLatest');box.replaceChildren();if(!data.latest?.length){box.appendChild(el('div','Ejecuta bun run sync:energia para cargar CNE.','empty-state'));return}for(const x of data.latest.slice(0,4)){const row=el('div',null,'series-row');const info=el('div');info.append(el('b',x.title||x.metric),el('small',[x.region,x.observed_at?.slice?.(0,10)].filter(Boolean).join(' · ')));row.append(info,el('strong',formatValue(x.value_number)));box.appendChild(row)}}
...
async function loadMap(){if(document.hidden)return;const geo=await fetch('/api/map/features').then(r=>r.json());allFeatures=geo.features||[];renderMap($('#mapLayer').value)}
...
function startTimers(){stopTimers();dashboardTimer=setInterval(()=>loadDashboard().catch(console.error),60000);progressTimer=setInterval(()=>loadProgress().catch(console.error),5000);mapTimer=setInterval(()=>loadMap().catch(console.error),300000)}
function stopTimers(){if(dashboardTimer)clearInterval(dashboardTimer);if(progressTimer)clearInterval(progressTimer);if(mapTimer)clearInterval(mapTimer);dashboardTimer=progressTimer=mapTimer=null}
initMap();$('#mapLayer').addEventListener('change',e=>renderMap(e.target.value));document.addEventListener('visibilitychange',()=>{if(document.hidden)stopTimers();else{loadDashboard().catch(console.error);loadProgress().catch(console.error);loadMap().catch(console.error);startTimers()}});initialLoad().then(startTimers).catch(console.error);
