...
async function renderElections(){
  ...
  try{
    ...
    elections.forEach((e,index)=>{
      ...
      const table=node('div',null,'election-table');const header=node('div',null,'election-row election-row-head');for(const h of['Candidatura','Partido / pacto','Votos','% válidos'])header.append(node('span',h));table.appendChild(header);
      for(const c of e.candidates){const row=node('div',null,'election-row'),candidate=node('div',null,'election-candidate'),name=node('b',c.name);if(c.elected===true)name.append(node('em','Electo/a','elected-badge'));candidate.append(name);if(c.ballotNumber)candidate.append(node('small',`Nº ${c.ballotNumber}`));const political=node('div',null,'election-political');political.append(node('b',c.party||c.list||'Independencia/partido no informado'));if(c.coalition)political.append(node('small',c.coalition));const vote=node('strong',num(c.votes)),share=node('div',null,'election-share');share.append(node('strong',pct(c.validVotePct)));const bar=node('i',null,'election-bar');bar.style.setProperty('--share',`${Math.max(0,Math.min(100,Number(c.validVotePct)||0))}%`);share.append(bar);row.append(candidate,political,vote,share);table.appendChild(row)}
      card.appendChild(table);const note=node('p','% válidos = votos de la candidatura / suma de votos de candidaturas válidas en este territorio. Nulos y blancos no forman parte de ese denominador.','election-note');card.appendChild(note);wrap.appendChild(card)
    });
